...
static Error * Start(ChatServer *self) {
	...
	// 待受用ソケット作成
	const int socket_listen = socket(PF_INET, SOCK_STREAM, 0);
		err = NewError(socket_listen == -1);
		if (err->IsError(err)) { err->SetMessage(err, "socket failed."); return err; }

	// 名前を付与
	int result_ = bind(socket_listen, (struct sockaddr *)(&addr_in), sizeof(addr_in));
		err = NewError(result_ == -1);
		if (err->IsError(err)) { err->SetMessage(err, "bind failed."); return err; }

	// 待受開始
	result_ = listen(socket_listen, 5);
		err = NewError(result_ == -1);
		if (err->IsError(err)) { err->SetMessage(err, "listen failed."); return err; }
...
extern Return_NewChatServer * NewChatServer(const int port) {
	...
	ChatServer *svr = (ChatServer *)(malloc(sizeof(ChatServer)));
		...
		if (err->IsError(err)) { err->SetMessage(err, "malloc failed."); return &(Return_NewChatServer){ err }; }
